...
export default function SharedReader({ token }: { token: string }) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      setDownloadError("PDF not available yet. The author may not have generated it.");
    } finally {
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        <div className="mt-6 flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-text-muted">
            {data.has_pdf ? "Free download — no account needed" : "PDF not yet generated by the author"}
          </p>
          {downloadError && (
            <p className="text-xs text-amber-500">{downloadError}</p>
          )}
        </div>
      </main>
